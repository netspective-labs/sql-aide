...
Deno.test("delimitedText rows streaming", async () => {
  const syntheticDT = delimitedText<
    RowFixture01,
    TabularContentObjStrategy<RowFixture01>
  >(
  ...
  const rows: RowFixture01[] = [];
  const rs = syntheticDT.rowsStream();
  for await (const row of rs.stream) {
    rows.push(row);
  }
  rs.close?.();
  assertEquals(rows, jsonFixture01);
});

Deno.test("delimitedText rows array", async () => {
  const syntheticDT = delimitedText<
    RowFixture01,
    TabularContentObjStrategy<RowFixture01>
  >(
  ...
  const rows = await syntheticDT.rows();
  assertEquals(rows, jsonFixture01);
});
