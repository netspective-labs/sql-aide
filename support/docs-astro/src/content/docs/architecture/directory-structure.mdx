...
### `pre-process`
The `pre-process` subdirectory contains a PostgreSQL `psql` pre-processor, which excels at handling meta commands like `\set` and `\include`. This pre-processor significantly enhances SQLa's capabilities by providing `psql` template features to databases that lack such functionality, such as SQLite, DuckDB, Clickhouse _local_, and Dolt. By leveraging the pre-processor, you can enjoy advanced features like variable substitution and file inclusion, improving your SQL generation workflow across different database systems.
